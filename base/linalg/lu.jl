...
# StridedMatrix
function lufact!{T<:BlasFloat}(A::StridedMatrix{T})
    lpt = LAPACK.getrf!(A)
    return LU{T,typeof(A)}(lpt[1], lpt[2], lpt[3])
end
lufact!(A::StridedMatrix) = generic_lufact!(A)
function generic_lufact!{T}(A::StridedMatrix{T})
    m, n = size(A)
    minmn = min(m,n)
    info = 0
...
lufact{T<:BlasFloat}(A::AbstractMatrix{T}) = lufact!(copy(A))
lufact{T}(A::AbstractMatrix{T}) = (S = typeof(one(T)/one(T)); S != T ? lufact!(convert(AbstractMatrix{S}, A)) : lufact!(copy(A)))
...
# See dgttrf.f
function lufact!{T}(A::Tridiagonal{T})
    n = size(A, 1)
    info = 0
